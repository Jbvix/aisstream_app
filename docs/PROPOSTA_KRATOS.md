...
> Python + Passenger WSGI, deploy via Git Version Control puxando do GitHub).
...
**Frontend:**
- `frontend/index.html` (~3.347 linhas) — mapa Leaflet ao vivo com posições, legendas, painéis flutuantes, vento, trilhas, monitor de fluxo BG.
- `frontend/dashboard.html` (~974 linhas) — geofences, manobras SAA, frota SAAM, market share, assistente, monitor de alterações.
...
- **Trilhas (rastros)** das embarcações — já existem; expandir para leitura de intenção (vetor de rumo/velocidade).
...
| Fase | Entrega | Foco | Status |
|------|---------|------|--------|
| **0 — Pipeline** | Pull GitHub → cPanel (Git Version Control) configurado | Deploy confiável | ✅ Concluído |
| **1 — Identidade KRATOS** | HUD de presença, feed de eventos, rebranding do assistente | UX / percepção | Próximo |
...
| **4 — Dados ricos** | Maré oficial, heatmap, vetor de rumo, score de confiança | Profundidade | — |
...
- **cPanel/Passenger**: app é long-running com WebSocket — em produção já se força *polling* quando o WSS falha (commit `12e41ca`).
...
1. **Proposta** (concluída — este documento)
2. **Pull GitHub → cPanel** (concluído — ver `DEPLOY_PULL_CPANEL.md`)
